...
function drawChart(chartID,jobTitle,jobCity,jobState) {
	...
	var params = {chartID:chartID,title:jobTitle,city:jobCity,state:jobState};
	var apiDataUrl = "http://hackathon.com/payscalecalc/api/payscale?" + jQuery.param( params );
	//http://hackathon.com/payscalecalc/api/payscale?city=new%20york&state=NY&title=Architect
...
function addJobDetailContainer() {
	...
	$('#search-results-list li').each(function() {				
		$(this).append('<div class="sr-details"><a href="#" class="btn-sr-detail">Details</a><div><p><strong>Details about ' + $(this).find('h2').text() + '.</strong> This paragraph can contain general hightlights that compare this position to similar job titles nationally. The average salary chart below is one example of on-demand external API data that could be represented in this context to provide the user with valuable insight.</b></p><div id="chart_div"></div></div></div>');
	});	
}
